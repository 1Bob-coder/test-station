	...
	Switch $nMsg
		...
		Case $hRunTests
			If StringCompare($sIpAddress, "") Then
				...
				If _IsChecked($hClosedCaptions) Then  ; If  closed captions box is checked
					RunClosedCaptionTest($hTestSummary, $hClosedCaptions_pf)              ; then run the closed captions test
				EndIf
...
; Inputs:  whichBox - 1 to 7
;          BoxIPAddress - Text Box to show the IP address.
Func BoxUnderTest($whichBox, $hBoxIPAddress)
	...
	$iBoxNum = $whichBox - 1
	FindBoxIPAddress($hBoxIPAddress)
